...
public struct JoinOf2<V1, V2> {
	...
	public func fulfill<NewValue>(
		...
		let promise = self.p1.then { v1 in return self.p2.then(on: context ?? self.p1.context) { return try transform(v1, $0) } }
        return promise
	}
...
public struct JoinOf3<V1, V2, V3> {
    ...
    public func fulfill<NewValue>(
        ...
        return parent.fulfill(on: context) { return ($0, $1) }.then { values in
...
public struct JoinOf4<V1, V2, V3, V4> {
	...
	public func fulfill<NewValue>(
        ...
        return parent.fulfill(on: context) { return ($0, $1, $2) }.then { values in
...
public struct JoinOf5<V1, V2, V3, V4, V5> {
	...
	public func fulfill<NewValue>(
        ...
        return parent.fulfill(on: context) { return ($0, $1, $2, $3) }.then { values in
	...
	public func fulfill<NewValue>(
        ...
        return parent.fulfill { return ($0, $1, $2, $3) }.then { values in
			let promise = self.promise.then(on: context) { return try transform (values.0, values.1, values.2, values.3, $0) }
            return promise
		}
	}
...
extension Promise {
    ...
    public func join<V2, V3, V4>(
        ...
        return self.join(p2).join(p3).join(p4)
    }
    ...
    public func join<V2, V3, V4, V5>(
        ...
        return self.join(p2).join(p3).join(p4).join(p5)
    }
