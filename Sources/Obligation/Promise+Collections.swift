...
extension Collection where IndexDistance == Int {
    ...
    public func map<NewValue>(
        ...
        return concurrencyLimiter (
            ...
            sources: self,
            preProcess: { (_, value, done, error) in try! done(value) },
            getPromise: { _, value in return try mapper(value) },
            transform: { _, _, value in return value },
            postProcess: { values in return values.map{$0!} },
            on: context
        );
    ...
    public func filter(
        ...
        return concurrencyLimiter (
            ...
            sources: self,
            preProcess: { _, value, done, error  in try! done(value) },
            getPromise: { _, value in return try filter(value) },
            transform: { (_, arg, value) -> Iterator.Element? in return value ? arg : nil },
            postProcess: { values in return values.filter{$0 != nil}.map{$0!} },
            on: context
        );
    }
}
...
extension Collection where Iterator.Element : PromiseProtocol, IndexDistance == Int {
    ...
    public func map<NewValue>(
        ...
        return concurrencyLimiter (
            ...
            getPromise: { (_, value: Iterator.Element.ValueType) -> Promise<NewValue> in return try mapper(value) },
            transform: { (_, _, value: NewValue) -> NewValue in return value },
            postProcess: { values in return values.map{$0!} },
            on: context
        );
    ...
    public func filter(
        ...
        return concurrencyLimiter (
            ...
            getPromise: { _, value in return try filter(value) },
            transform: { (_, arg, value) -> Iterator.Element.ValueType? in return value ? arg : nil },
            postProcess: { values in return values.filter{$0 != nil}.map{$0!} },
            on: context
        );
