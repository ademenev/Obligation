	...
	override func spec() {
        ...
        describe("'firstly'") {
            ...
            it ("creates a rejected promise if block throws") {
                expectError {
                    return firstly(on: context) { () -> Int in
                        throw MockError()
                    }
                }
            }
		...
		describe("Promise") {
			it("is initialized in pending state") {
				...
				let promise2 = Promise<String>() { fulfill, reject in
				}
			...
			it ("can recover from errors") {
				let promise = Promise<Int> {fulfill, reject in
					...
					throw MockError2()
				}.recover(type: MockError2.self) { (error) -> Int in
					throw MockError()
				}.recover(type: MockError.self) { (error) -> Int in
					return 100
				}
			...
			it("propagates context") {
				...
				Promise<Int>(on: mainContext) {fulfill, reject in
					checkContext("mainContext")
					fulfill(1)
				}.then() { (value) -> Int in
					checkContext("mainContext")
					return value
				}.then(on: otherContext) { (value) -> Int in
			...
			it("can change context") {
				...
				Promise<Int>(on: firstContext) { fulfill, reject in
					checkContext("firstContext")
					reject(MockError())
				}.recover(on: secondContext) { (error) -> Int in
				...
				.then() { (value) -> Int in
					checkContext("secondContext")
					throw MockError()
				}.recover(on: firstContext) { (error) -> Int in
					checkContext("firstContext")
					return 1000
				}.then() { (value) -> Int in
					checkContext("secondContext")
			...
			it ("can handle specific errors") {
				...
				.catch(type: MockError.self) { error in
					handled1 = true
				}
				
				Promise<Int>.rejected(MockError())
				.catch(type: MockError2.self) { error in
				}.catch { error in
            ...
            describe("when original promise is fulfilled") {
                it ("is fulfilled after returned promise is fulfilled") {
                    noErrors { () -> Promise<Void> in
                        ...
                        let promise = Promise.fulfilled(on: context, 10)
                        return promise.finally {
                            return Promise.fulfilled(1).then { value in
                                finallyModified += 1
                            }
                        }.then { value in
                            finallyModified += 1
